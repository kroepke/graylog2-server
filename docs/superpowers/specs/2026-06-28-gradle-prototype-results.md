...
| 4 | **Cross-run cache** — `gradle clean` then recompile | **~2.9 s** (4 FROM-CACHE, 3 extracted) | No equivalent cross-run cache in Maven | `compileJava`, `generateProto`, `generateGrammarSource`, `generateOpampProto` all restore from cache. Full `jar` after `clean`: **~11 s** (4 FROM-CACHE, 5 UP-TO-DATE, 6 executed; yarnBuild UP-TO-DATE). |
